...
export const ConfigSchema = z.object({
  ...
  max_request_retries: z.number().positive().optional().default(3),
...
export interface Config {
   ...
   *
   * Controls how many times the crawler will retry a failed request before giving up.
   * This applies to both page crawling and document indexing operations.
   *
   * @default 3
   */
  max_request_retries?: number | null
  ...
  /** Enhanced webhook configurations
   *
   * Advanced webhook configuration that supports multiple webhook endpoints
   * with different authentication methods, event filtering, and retry logic.
   *
   * Example:
   * ```ts
   * webhooks: {
   *   "main": {
   *     url: "https://api.example.com/webhooks/scrapix",
   *     auth: {
   ...
   *         algorithm: "sha256",
   *         header: "X-Scrapix-Signature"
   *       }
   *     },
   *     events: ["progress.update", "page.error"],
   *     enabled: true
   *   }
   * }
   * ```
