...
export class Sender {
  ...
  async finish() {
    ...
    if (this.pendingTasks && this.pendingTasks.length > 0) {
      log.info(
        `Waiting for ${this.pendingTasks.length} pending tasks to complete`
      )
      try {
        await Promise.all(
    ...
    await this.webhook.completed(this.config, {
      nbDocumentsSent: this.nb_documents_sent,
    })
  ...
  async __batchSend(): Promise<void> {
    ...
    try {
      ...
      const batchSizeBytes = JSON.stringify(this.queue).length
      ...
      this.eventBus.emitBatchSent({
        documentCount: queueSize,
        success: true,
        duration: batchDuration,
        indexUid: this.index_uid,
        taskId: task.taskUid,
        batchSizeBytes,
        totalDocumentsSent: this.nb_documents_sent + queueSize,
        isRetry: (this.retryCount || 0) > 0,
        retryAttempt: this.retryCount || 0,
      })
      ...
      // Emit batch sent event for error case
      this.eventBus.emitBatchSent({
        documentCount: queueSize,
        success: false,
        duration: batchDuration,
        ...
        totalDocumentsSent: this.nb_documents_sent, // Don't increment on failure
        isRetry: this.retryCount > 1,
      ...
      if (this.retryCount <= getConfig('RETRY', 'MAX_ATTEMPTS')) {
        log.info(
          `Retrying batch send (attempt ${this.retryCount}/${getConfig('RETRY', 'MAX_ATTEMPTS')})`
        )
        ...
        throw new ScrapixError(
          ...
          `Failed to send ${this.queue.length} documents to Meilisearch after ${getConfig('RETRY', 'MAX_ATTEMPTS')} retry attempts`,
          {
            ...
            lastError: error,
            suggestion:
              'Check Meilisearch server status and network connectivity',
          }
        )
      }
    } finally {
  ...
  async __batchSendSync() {
    ...
    try {
      ...
      this.nb_documents_sent += queueSize
      ...
      this.eventBus.emitBatchSent({
        ...
        taskId: task.taskUid,
        batchSizeBytes,
        totalDocumentsSent: this.nb_documents_sent,
        isRetry: false,
        retryAttempt: 0,
      })
