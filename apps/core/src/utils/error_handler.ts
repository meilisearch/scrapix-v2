...
export function formatError(error: unknown): {
  ...
  if (error instanceof Error) {
    ...
    if (error.message.includes('ECONNREFUSED')) {
      return {
        message:
          'Failed to connect to the service. Please check if the service is running and the URL is correct.',
    ...
    if (error.message.includes('ETIMEDOUT')) {
      return {
        message:
          'Request timed out. The service might be overloaded or the network is slow.',
