...
export const CRAWLER_CONSTANTS = {
  ...
  AI: {
    ...
    MAX_CONTENT_LENGTH: parseInt(
      process.env.SCRAPIX_AI_MAX_CONTENT_LENGTH || '4000',
      10
    ),
    ...
    EXTRACTION_TEMPERATURE: parseFloat(
      process.env.SCRAPIX_AI_EXTRACTION_TEMP || '0.1'
    ),
    SUMMARY_TEMPERATURE: parseFloat(
      process.env.SCRAPIX_AI_SUMMARY_TEMP || '0.3'
    ),

    /** Maximum tokens for AI summary */
    SUMMARY_MAX_TOKENS: parseInt(
      process.env.SCRAPIX_AI_SUMMARY_MAX_TOKENS || '150',
      10
    ),

    /** Request timeout for AI API calls (ms) */
    REQUEST_TIMEOUT: parseInt(
      process.env.SCRAPIX_AI_REQUEST_TIMEOUT || '30000',
      10
    ),
  },
  ...
  MEILISEARCH: {
    ...
    TASK_WAIT_TIMEOUT: parseInt(
      process.env.SCRAPIX_TASK_WAIT_TIMEOUT || '15000',
      10
    ),

    /** Extended timeout for finish operations (ms) */
    TASK_WAIT_TIMEOUT_EXTENDED: parseInt(
      process.env.SCRAPIX_TASK_WAIT_TIMEOUT_EXTENDED || '30000',
      10
    ),

    /** Timeout for initial index creation (ms) */
    INDEX_CREATE_TIMEOUT: parseInt(
      process.env.SCRAPIX_INDEX_CREATE_TIMEOUT || '5000',
      10
    ),
  },
  ...
  RATE_LIMIT: {
    ...
    CRAWL_MAX_REQUESTS: parseInt(
      process.env.SCRAPIX_RATE_LIMIT_CRAWL || '100',
      10
    ),

    /** Maximum requests per window for status endpoints */
    STATUS_MAX_REQUESTS: parseInt(
      process.env.SCRAPIX_RATE_LIMIT_STATUS || '60',
      10
    ),

    /** Maximum requests per window globally */
    GLOBAL_MAX_REQUESTS: parseInt(
      process.env.SCRAPIX_RATE_LIMIT_GLOBAL || '1000',
      10
    ),
  },
  ...
  CRAWLER: {
    ...
    DEFAULT_CONCURRENCY: parseInt(
      process.env.SCRAPIX_DEFAULT_CONCURRENCY || '10',
      10
    ),

    /** Default requests per minute limit */
    DEFAULT_REQUESTS_PER_MINUTE: parseInt(
      process.env.SCRAPIX_DEFAULT_RPM || '100',
      10
    ),
  },
  ...
  HTTP: {
    ...
    KEEP_ALIVE_MS: parseInt(
      process.env.SCRAPIX_HTTP_KEEP_ALIVE_MS || '1000',
      10
    ),

    /** Maximum sockets per host */
    MAX_SOCKETS_PER_HOST: parseInt(
      process.env.SCRAPIX_HTTP_MAX_SOCKETS || '256',
      10
    ),

    /** Maximum free sockets to keep around */
    MAX_FREE_SOCKETS: parseInt(
      process.env.SCRAPIX_HTTP_MAX_FREE_SOCKETS || '256',
      10
    ),
...
>(category: T, key: K): (typeof CRAWLER_CONSTANTS)[T][K]
