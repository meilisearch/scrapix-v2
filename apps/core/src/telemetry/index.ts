...
export function extractCustomerAttributes(config: Config): Record<string, any> {
  ...
  return {
    ...
    'config.features': Object.keys(config)
      .filter((k) => k.includes('_enabled') && config[k as keyof Config])
      .join(','),
  }
}
...
class TelemetryManager {
  ...
  async initialize(serviceName: string = 'scrapix-scraper') {
    ...
    console.log(
      `OpenTelemetry initialized for ${serviceName} in ${environment} mode`
    )
  }
...
export class BusinessMetrics {
  private crawlsCounter = telemetry.meter.createCounter(
    'scraper.customer.crawls.total',
    {
      description: 'Total number of crawls per customer',
    }
  )

  private pagesCounter = telemetry.meter.createCounter(
    'scraper.customer.pages.crawled',
    {
      description: 'Total pages crawled per customer',
    }
  )

  private documentsCounter = telemetry.meter.createCounter(
    'scraper.customer.documents.indexed',
    {
      description: 'Total documents indexed per customer',
    }
  )

  private openaiTokensCounter = telemetry.meter.createCounter(
    'scraper.openai.tokens.used',
    {
      description: 'OpenAI tokens consumed',
    }
  )

  private openaiCostCounter = telemetry.meter.createCounter(
    'scraper.openai.cost.estimated',
    {
      description: 'Estimated OpenAI cost in USD cents',
    }
  )

  private featureUsageCounter = telemetry.meter.createCounter(
    'scraper.customer.features.usage',
    {
      description: 'Feature usage per customer',
    }
  )

  private crawlDurationHistogram = telemetry.meter.createHistogram(
    'scraper.customer.crawl.duration',
    {
  ...
  recordOpenAIUsage(
    ...
    // Record tokens
    this.openaiTokensCounter.add(inputTokens, {
      ...attributes,
      'token.type': 'input',
    })
    this.openaiTokensCounter.add(outputTokens, {
      ...attributes,
      'token.type': 'output',
    })
