...
// Queue metrics
class QueueMetrics {
  private jobsAdded = telemetry.meter.createCounter(
    'scraper.queue.jobs.added',
    {
      description: 'Jobs added to queue',
    }
  )

  private jobsCompleted = telemetry.meter.createCounter(
    'scraper.queue.jobs.completed',
    {
      description: 'Jobs completed successfully',
    }
  )

  private jobsFailed = telemetry.meter.createCounter(
    'scraper.queue.jobs.failed',
    {
      description: 'Jobs that failed',
    }
  )

  private jobDuration = telemetry.meter.createHistogram(
    'scraper.queue.job.duration',
    {
  ...
  private queueDepth = telemetry.meter.createObservableGauge(
    'scraper.queue.depth',
    {
  ...
  private activeJobs = telemetry.meter.createObservableGauge(
    'scraper.queue.jobs.active',
    {
...
export class JobTelemetry {
  ...
  static async processWithTelemetry<T>(
    ...
    const span = telemetry.tracer.startSpan('queue.job.process', {
