...
export abstract class BaseCrawler {
  ...
  constructor(sender: Sender, config: Config) {
    ...
    // Initialize EventBus with configuration
    this.eventBus.initialize(config)
  }
  ...
  protected async handlePage(context: any): Promise<void> {
    ...
    this.eventBus.emitPageCrawled({
      ...
      totalCrawled: this.nb_page_crawled,
      totalIndexed: this.nb_page_indexed,
    })
    ...
    try {
      if (request.loadedUrl) {
        if (
          ...
          try {
            ...
            // Emit page indexed event
            this.eventBus.emitPageIndexed({
              url: request.loadedUrl,
              success: true,
              duration: processingDuration,
              documentCount: 1, // Assuming one document per page, adjust if needed
              featuresApplied: Object.keys(this.config.features || {}).filter(
                (key) => (this.config.features as any)?.[key]?.activated
              ),
              totalIndexed: this.nb_page_indexed,
            })
      ...
      // Calculate total page processing duration and emit final crawled event
      ...
      // Emit page error event for unexpected errors
      this.eventBus.emitPageError({
        url,
        error: error instanceof Error ? error : new Error(String(error)),
        stage: 'crawling',
      ...
      // Also emit failed page crawled event
      this.eventBus.emitPageCrawled({
        url,
        success: false,
        duration: pageDuration,
  ...
  protected __is_file_url(url: string): boolean {
    ...
    return Array.from(fileExtensions).some((extension) =>
      urlPath.endsWith(extension)
    )
  }
